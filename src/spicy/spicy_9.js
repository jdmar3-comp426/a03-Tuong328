...
export const multiplyBy = (num1) => {
    return function newFunction(num2) {return num1 * num2};
};
...
export const filter = (arr, test) => {
    const passFailObject = {fail:[], pass:[]};
    for (let i = 0; i < arr.length; i++) {
        if (test(arr[i])) {
            passFailObject.pass.push(arr[i]);
        } else {
            passFailObject.fail.push(arr[i]);
        }
    }
    return passFailObject;
};
...
export const allEvensAreOdd = (arr) => {
    return everyEven(arr, function odd(num) {return (num % 2 === 1)});
};
...
export const anEvenIsOdd = (arr) => {
    return someEven(arr, function odd(num) {return (num % 2 === 1)});
};
...
export const hasExactly = (arr, test, n) => {
    const filtered = filter(arr, test);
    return filtered.pass.length === n;
};
